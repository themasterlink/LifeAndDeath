...
bool Kroki::update(){
	if(m_energyLevel > 0 && m_isStillAlive){
		...
		if(chase == 0){ // chase food
			...
			const double updateAngle = std::min(m_maxUpdateAngle, differentAngle);
		...
		if(m_pos[0] < 10 && acos(dir.dot(dPoint2(-1, 0))) < M_PI / 2.){
			m_currentAngle += M_PI / 6.0;
		}else if(m_pos[0] > m_map.getSize()[0] - 11 && acos(dir.dot(dPoint2(1, 0))) < M_PI / 2.){
			m_currentAngle += M_PI / 6.0;
		}
		if(m_pos[1] < 10 && acos(dir.dot(dPoint2(0, -1))) < M_PI / 2.){
			m_currentAngle += M_PI / 6.0;
		}else if(m_pos[1] > m_map.getSize()[1] - 11 && acos(dir.dot(dPoint2(0, 1))) < M_PI / 2.){
			m_currentAngle += M_PI / 6.0;
		}
...
Kroki::Kroki(unsigned long id, Map& map, dPoint2 startPose, double startAngle, double speed, double sense, double size,
		std::vector<Kroki>* population)
		: m_isStillAlive(true), m_speed(speed), m_sense(sense), m_size(size),
		  m_pos(startPose), m_currentAngle(startAngle),
